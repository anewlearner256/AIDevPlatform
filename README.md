...
### 生产级能力（已落地基础版）
- [x] 可观测性基础：请求上下文（Request ID / Tenant ID）+ 系统指标接口 + 监控仪表板页面
- [x] CI质量门禁：GitHub Actions 中执行编译检查与单元测试
- [x] 多租户基础：`X-Tenant-ID` 请求头注入与租户上下文透传
- [x] 插件系统基础：插件抽象、注册中心、管理器与示例插件

> 注：上述模块多数已具备“可开发/可联调”的基础实现，部分高级能力（如完整RAG链路、技能系统、生产级稳定性）仍在持续完善。
